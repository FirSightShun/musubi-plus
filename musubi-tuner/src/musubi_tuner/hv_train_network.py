        ...
        for epoch in range(epoch_to_start, num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(training_model):
                    ...
                    # per-sample weighted loss (sample_weight from dataset, multiplier from args)
                    if "sample_weight" in batch:
                        loss = loss.mean(dim=list(range(1, loss.ndim)))  # [B] per-sample loss
                        sample_w = batch["sample_weight"].to(loss.device, loss.dtype) * args.sample_weight_multiplier
...
def setup_parser_common() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to JSON file mapping sample stems to weights (for per-sample weighted loss) / サンプル重み付き損失用のJSONファイルパス",
    )
    parser.add_argument(
        "--sample_weight_multiplier",
        type=float,
        default=1.0,
        help="Global multiplier for sample weights (default: 1.0) / サンプル重みのグローバル倍率（デフォルト: 1.0）",
    )
